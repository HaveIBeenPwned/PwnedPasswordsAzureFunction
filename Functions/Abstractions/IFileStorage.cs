...
public interface IFileStorage
{
    ...
    Task<PwnedPasswordsFile> GetHashFileAsync(string hashPrefix, HashType mode, CancellationToken cancellationToken = default);

    /// <summary>
    /// Updates a hash file using the hash prefix.
    /// </summary>
    /// <param name="hashPrefix">The hash prefix to use to look up the file.</param>
    /// <param name="mode">The mode to get, either <see cref="HashType.SHA1"/> or <see cref="HashType.NTLM"/></param>
    ...
    Task<bool> UpdateHashFileAsync(string hashPrefix, HashType mode, SortedDictionary<string, int> hashes, string etag, CancellationToken cancellationToken = default);
